...
class SyncSaver(object):
    ...
    def __init__(self, filename, systematics="NA"):
        ...
        if config.leptons == 1:
            ...
            raise ValueError("invalid configuration with lepton count {}".format(config.leptons))
    ...
    def _print_lepjets(self, event, ws):
        def dtos(d):
            s = "{0:.4f}".format(d)
            return "0" if s == "0.0000" else s
        leptons = event.leptons()
        jets = event.jets(self.__sys)
        met = event.met(self.__sys)
        first = leptons[0]
        ...
        s = ",".join(map(str,
                         [
                             ...
                             1, 0,
                             dtos(first.p4().pt()), dtos(first.p4().eta()), dtos(
                                 first.p4().phi()), dtos(first.relativeIsolation()), first.pdgId(),
                         ] + [0] * 7 + jpts + jcsvs + map(dtos, [met.pt(), met.phi()]) + [0, len(jets), len(filter(btag, jets)), dtos(ws['csvweight']), event.hfCategory()]
                         ))
        self.__f.write(s + "\n")

    def _print_dilepton(self, event, ws):
        leptons = event.leptons()
        ...
        self.__f.write(self.__fmt % (
            ...
            event.met().pt(), event.met().phi(), len(event.jets())))
