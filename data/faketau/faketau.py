...
Leaf('lepton_pt', '[f]', return_list.format('leptons', 'p4().Pt()'))
Leaf('lepton_eta', '[f]', return_list.format('leptons', 'p4().Eta()'))
Leaf('lepton_fakeable', '[i]', return_list.format('leptons', 'fakeable()'))
Leaf('lepton_mva', '[i]', return_list.format('leptons', 'mva()'))

Leaf('lepton_genparticle_pt', '[f]', """
     for (const auto& lepton: leptons) {
         auto p = lepton.genParticle();
         if (not p) {
...
Leaf('lepton_genparticle_eta', '[f]', """
     ...
     """)
Leaf('lepton_genparticle_closestpt', '[f]', """
     for (const auto& lepton: leptons) {
         auto p = lepton.genParticle();
         if (not p) {
             result.push_back(-666.);
         ...
         auto j = lepton.genParticle()->closestGenJet();
         if (j) result.push_back(j->pt());
         else result.push_back(-666.);
     }
     """)
Leaf('lepton_genparticle_closestdr', '[f]', """
     for (const auto& lepton: leptons) {
         auto p = lepton.genParticle();
         if (not p) {
             result.push_back(-666.);
         ...
         auto j = lepton.genParticle()->closestGenJet();
         if (j) result.push_back(dR(j->p4(), lepton.genParticle()->p4()));
         else result.push_back(-666.);
     }
     """)
Leaf('lepton_genparticle_closestparticlept', '[f]', """
     for (const auto& lepton: leptons) {
         auto p = lepton.genParticle();
         if (not p) {
             result.push_back(-666.);
         ...
         auto j = lepton.genParticle()->closestGenParticle();
         if (j) result.push_back(j->pt());
         else result.push_back(-666.);
     }
     """)
Leaf('lepton_genparticle_closestparticledr', '[f]', """
     for (const auto& lepton: leptons) {
         auto p = lepton.genParticle();
         if (not p) {
             result.push_back(-666.);
...
Leaf('genparticle_pt', '[f]', return_list.format('event.genParticles()', 'p4().Pt()'))
Leaf('genparticle_eta', '[f]', return_list.format('event.genParticles()', 'p4().Eta()'))

Leaf('genparticle_closestpt', '[f]', """
     for (const auto& jet: event.genParticles()) {
         auto j = jet.closestGenJet();
         if (j) result.push_back(j->pt());
         else result.push_back(-666.);
     }
     """)
Leaf('genparticle_closestdr', '[f]', """
     for (const auto& jet: event.genParticles()) {
         auto j = jet.closestGenJet();
         if (j) result.push_back(dR(j->p4(), jet.p4()));
         else result.push_back(-666.);
     ...
     """)
Leaf('genparticle_closestparticlept', '[f]', """
     for (const auto& jet: event.genParticles()) {
         auto j = jet.closestGenParticle();
         if (j) result.push_back(j->pt());
         else result.push_back(-666.);
     ...
     """)
Leaf('genparticle_closestparticledr', '[f]', """
     for (const auto& jet: event.genParticles()) {
         auto j = jet.closestGenParticle();
         if (j) result.push_back(dR(j->p4(), jet.p4()));
         else result.push_back(-666.);
