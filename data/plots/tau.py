...
Plot(
    name="taus/TT_DeltaR",
    code="histo.Fill(dR(taus[0], taus[1]))",
    labels=["#DeltaR #tau_{1,2}", "Events"],
    binning=[15, 0, 6.28]
)
Plot(
    name="taus/TT_VisibleMass",
    code="histo.Fill((taus[0].p4() + taus[1].p4()).M())",
...
for n in range(2):
    ...
    Plot(
            ...
            code="histo.Fill(abs(taus[{0}].pdgId()))".format(n),
            labels=[lbl + " id", "Events"],
            binning=[30, 0, 30]
    )
    Plot(
            name="taus/generator/T{0}_ParentMatch".format(n + 1),
            code="histo.Fill(abs(taus[{0}].parentId()))".format(n),
